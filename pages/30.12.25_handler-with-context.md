...
One thing that I find is missing from Go's `http` library is some sort of context passed through the request handler chain. Obviously there is a `context.Context` attached to the `Request` given, but using a `any` map to access data is cumbersome and not very 'clean'.
