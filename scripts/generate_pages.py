...
def filename_to_title(filename: str) -> str:
    ...
    return " ".join(name.removesuffix(".md").capitalize().split("-"))
...
def replace_image_template(html):
    ...
    if DEV:
        ...
        replaced_html = re.sub(pattern, r'<img src="/curiosities/assets/\1">', html)
    ...
    with open(f"pages/{filename}", "r") as f:
        ...
        if filename == "index.md":
            ...
            for file in files:
                ...
                file_link = (
                    f"{filename_to_date(file)} [{filename_to_title(file)}](src/{file})"
                )
                entries += f"- {file_link}\n"
